...
async def async_test(douyin_id: str = None) -> None:
    douyin_data = await api.get_douyin_video_data(douyin_id)
    with open('./video_info_json/info_{}_{}.json'.format(keyword, video_id), 'w', encoding='utf-8') as f:
                json.dump(douyin_data, f, ensure_ascii=False)

import time
if __name__ == '__main__':
    api = Scraper()
    # 运行测试
    os.chdir('E:/CS/ICH video')
    path = './video_id_json/'
    count = 0
    flag = False
    for file_name in os.listdir(path):
        ...
        if flag:
            ...
            for video_id in list(video_id_json[keyword]):
                ...
                time.sleep(3.5)
                print(video_id)
                asyncio.run(async_test(douyin_id=video_id))
                count += 1
                if count >2000:
                    time.sleep(300)
                    count = 0
